...
use syn::{Stmt, BinOp, punctuated::Punctuated, Expr, token::Comma, ExprAssign, Member, ExprRange, Lit, Local, Pat, UnOp, Type};
...
pub fn local_to_ident(local: &Local) -> String {
    ...
    ident + "_is_" + expr_to_ident(&local.init.as_ref().expect("When `let` statements are expected to have an initial value").1).as_str()
}

pub fn pat_to_ident(pat: &Pat) -> Ident {
    match pat {
        Pat::Ident(ref pat_ident) => pat_ident.ident.clone(),
        _ => todo!("Expected Pat::Ident, got {:?}", pat),
    }
}

pub fn expr_to_ident(expr: &syn::Expr) -> String {
    match expr {
        Expr::Array(array) => if array.elems.is_empty() { "empty_array".to_string() } else { punctuated_to_ident(&array.elems)},
        Expr::Assign(assign) => expr_assign_to_ident(assign),
        Expr::AssignOp(assign_op) => expr_to_ident(&assign_op.left) + "_" + binary_op_to_ident(&assign_op.op) + "_" + &expr_to_ident(&assign_op.right),
        Expr::Async(_) => todo!("Async not supported"),
        Expr::Await(_) => todo!("Await not supported"),
        Expr::Binary(binary) => expr_to_ident(&binary.left) + "_" + binary_op_to_ident(&binary.op) + "_" + &expr_to_ident(&binary.right),
        ...
        Expr::Call(call) => expr_to_ident(&call.func) + if !call.args.is_empty() {
        ...
        Expr::Field(field) => expr_to_ident(&field.base) + "_" + member_to_ident(&field.member).as_str(),
        ...
        Expr::MethodCall(method_call) => expr_to_ident(&method_call.receiver) + "_" + method_call.method.to_string().to_lowercase().as_str() + if !method_call.args.is_empty() {
        ...
        Expr::Unary(unary) => unary_op_to_ident(&unary.op).to_string() + "_" + &expr_to_ident(&unary.expr),
...
fn type_to_ident(ty: &Type) -> String {
    match ty {
        Type::Array(array) => format!("array_of_{}_{}", expr_to_ident(&array.len), type_to_ident(&array.elem)),
        Type::BareFn(_) => todo!(),
        Type::Group(_) => todo!(),
        ...
        Type::Never(_) => todo!(),
        Type::Paren(_) => todo!(),
        Type::Path(path) => path.path.segments.last().unwrap().ident.to_string().to_lowercase(),
        Type::Ptr(_) => todo!(),
...
pub fn path_to_ident(path: &syn::ExprPath) -> String {
    path.path.segments.last().expect("Expected path to have at least 1 segment").ident.to_string().to_lowercase()
}

fn block_to_ident(block: &syn::ExprBlock) -> String {
    block.block.stmts.last().map(stmt_to_ident).unwrap_or_else(|| "noop".to_string())
}
...
pub fn punctuated_to_ident(punctuated: &Punctuated<Expr, Comma>) -> String {
    punctuated.iter().map(expr_to_ident).collect::<Vec<String>>().join("_")
}
...
fn expr_lit_to_ident(lit: &syn::ExprLit) -> String {
    match &lit.lit {
        ...
        Lit::Byte(_) => todo!(),
        Lit::Char(_) => todo!(),
        Lit::Int(value) => convert(value.base10_parse::<i64>().unwrap(), Formatting { dashes: false, title_case: false, ..Formatting::default() }),
        Lit::Float(_) => todo!(),
